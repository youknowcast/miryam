...
- 左サイドバーは **目次 / 注釈 / 検索** の 3 タブです
  ...
  - **注釈**: メモを書きます (入力は即座に保存され、保存操作はありません)。カンマ区切りでタグを付けられ、入力欄にフォーカスすると同じ PDF 内の既存タグが候補として出ます。上部のドロップダウンでタグによる絞り込みができます (絞り込み中でも編集自体はでき、一覧への反映は次に開き直すか注釈が増減したタイミングです)
