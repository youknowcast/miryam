...
`colors` (マーカー色) と `recall_probability` (読書の感想を定期発話に混ぜる確率) も
設定項目として存在し値の検証はされますが、現状の reader はマーカー色を内部で
固定しており `phrases.toml` を読み込まないため、**この 2 つは書いても今は適用されません**
(将来のフェーズで使う予定の予約項目です)。

- 右クリック「本棚」→ 一覧から行をクリックすると別ウィンドウ (`miryam-reader`) で開きます
- 一覧の各行は `ファイル名  マーカー N / メモ M  p.K  MM/DD` (K はしおりのページ、
  MM/DD は最終閲覧日) の形です。未読 (一度も開いていない) の行は `ファイル名  未読` になります
