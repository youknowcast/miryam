...
`colors` (マーカー色)・`recall_probability` (読書の感想を定期発話に混ぜる確率)・`book`
(注釈の書き出し先ノートブック名) の 3 つも `ReaderConfig` として値の検証はされますが、
Phase 1 の reader はまだこれらを読みません — マーカー色は内部で固定しており、感想の
定期発話や Inkdrop への書き出しはまだ実装していないためです。書いても現時点では
無視されます (将来のフェーズで使う予定の予約項目です)。
...
- 左の注釈一覧でメモを書きます (入力は即座に保存され、保存操作はありません)
...
現時点では全文検索・目次・タグ・LLM への質問・Inkdrop への書き出しは未実装です。
