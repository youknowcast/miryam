...
ときの色ポップオーバーの選択肢に反映されます。`recall_probability` (読書の感想を
定期発話に混ぜる確率)・`book` (注釈の書き出し先ノートブック名) の 2 つも
`ReaderConfig` として値の検証はされますが、reader はまだこれらを読みません
(将来のフェーズで使う予定の予約項目です)。
