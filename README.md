...
- 左サイドバーは **目次 / 注釈 / 検索** の 3 タブです
  ...
  - **検索**: `Ctrl+F` で検索タブが開き、検索欄にフォーカスが移ります。探すのは **PDF 本文だけ**です (自分で書いたメモやマーカーの引用文は対象になりません)。大文字小文字は区別しません。Enter で検索・次の一致へ移動、Shift+Enter で前の一致へ。ページ数の多い本でもメインループを止めないよう少しずつ走査するため、一致は見つかった順に一覧へ並んでいきます。一致箇所はページ上にオレンジ色で強調されます
